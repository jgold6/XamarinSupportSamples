	...
	partial class LongRunningTaskViewController : UIViewController
	{
        ...
        async partial void OnStartTask(UIButton sender)
        {
            ...
            taskId = UIApplication.SharedApplication.BeginBackgroundTask(() => {
				// Called when background time is up
				Console.WriteLine("Background time is up.");
				// Add cancellation support
				cts.Cancel();
			});
            ...
            try {
                await Task.Run(() => {
                    for (long count = 1; running == true ; count++) {
                        ...
                        Thread.Sleep(1000);
						Console.WriteLine("Remaining Seconds: " + UIApplication.SharedApplication.BackgroundTimeRemaining);
                    }
